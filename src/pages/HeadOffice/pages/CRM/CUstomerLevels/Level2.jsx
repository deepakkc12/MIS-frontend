...
const Level2Customer = () => {
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const response = await getRequest(`crm/live-customer-list/?level=2`);
      if (response.data) {
  ...
  const Navbar = () => (
    <div className="flex justify-between items-center mb-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Gold Customer Analytics</h1>
        <p className="text-gray-500">Top customers contributing to second 25% of total sales</p>
      </div>
      
      {/* <div className="flex space-x-3">
        <div className="relative inline-block">
          <button className="bg-white border border-gray-200 rounded-lg px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 flex items-center">
        ...
        <button 
          onClick={() => getData()} 
          className="bg-white border border-gray-200 rounded-lg px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 flex items-center"
        >
  ...
  const CustomerTable = () => (
    ...
    //     <h2 className="text-xl font-semibold text-gray-800">Level 1 Customer Details</h2>
    ...
    //           <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Contrib. (%)</th>
    //           <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Cumulative (%)</th>
    //         </tr>
    //       </thead>
    //       <tbody>
    //         {customerData.map((customer, index) => (
    //           <tr key={customer.Code} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
    ...
    //               {parseFloat(customer.Last2MthSales || 0).toLocaleString(undefined, {maximumFractionDigits: 2})}
    //             </td>
    //             <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
    //               <span className="bg-blue-100 text-blue-800 font-medium py-1 px-2 rounded-full text-xs">
    //                 {(parseFloat(customer.contribution || 0) * 100).toFixed(2)}%
    //               </span>
    //             </td>
    //             <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
    ...
    //                     style={{ width: `${parseFloat(customer.cumulativeContribution)}%` }}
    //                   ></div>
    //                 </div>
    //                 <span className="text-gray-900 font-medium">
    //                   {parseFloat(customer.cumulativeContribution || 0).toFixed(2)}%
    //                 </span>
    //               </div>
    //             </td>
    //           </tr>
    ...
    //       Showing <span className="font-medium">{customerData.length}</span> customers
    //     </div>
    //     <div className="flex">
    //       <button className="px-3 py-1 border border-gray-300 rounded-md mr-2 text-sm text-gray-600">Previous</button>
    //       <button className="px-3 py-1 bg-indigo-600 text-white rounded-md text-sm">Next</button>
    //     </div>
    //   </div>
    // </div>

    <TableLayout title='Customer List' data={customerData} headers={headers}/>
  );
