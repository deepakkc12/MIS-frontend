...
const GroupAnalatycs = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await getRequest(
          `inventory/group-details/?productGpCode=${gpCode}`
        );
  ...
  const brandSalesData = data.brands
    ...
    .map((brand) => ({
      ...
      growth: parseFloat(brand.PrevMonthSales)
        ? (
            ((parseFloat(brand.CurrentMonthSales) -
              parseFloat(brand.PrevMonthSales)) /
              parseFloat(brand.PrevMonthSales)) *
            100
          ).toFixed(1)
        : 0,
  ...
  const skuPerformanceData = data.Skus.filter(
    ...
    .map((sku) => {
      const brandInfo = data.brands.find(
        (brand) => brand.BrandCode === sku.brandCode
      );
  ...
  const navigateToBrandDetail = (brand) => {
    ...
    navigate(`/inventory/gNb/${gpCode}/${brand.BrandCode}`)
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-1">
              <nav className="flex space-x-2">
                ...
                >
                  <svg
                    className={`mr-2 h-4 w-4 ${
                      activeTab === "brands" ? "text-blue-600" : "text-gray-400"
                    }`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
