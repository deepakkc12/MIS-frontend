...
import OpexSalesComparison from "./OpexComparison";
import { lastUpdated } from "../../../../../utils/constants";

const SalesOverView = () => {
  // State management
  const [activeTab, setActiveTab] = useState("daily");
  const [salesTarget,setSalesTargets] = useState([])
  ...
  const getSalesTargets =async()=>{
    const response = await getRequest(`sales/targets/`)
    if(response.success){
      setSalesTargets(response.data)
    }
  }
  ...
  const getCombinedData = () => {
    if (!comparisonMode || !data || !prevData) {
      // Regular data flow (no comparison)
      const baseData = data || [];
      
      // Get formatted targets
      const targets = formatSalesTargets();
      const periodKey = getXAxisKey();
      
      // Merge targets with sales data
      return baseData.map(item => {
        const matchingTarget = targets.find(target => target[periodKey] === item[periodKey]);
        return {
          ...item,
          target: matchingTarget ? matchingTarget.target : null
        };
      });
    }
  
    // Handle comparison mode (your existing logic)
    const activeViewType = activeTab; // 'daily', 'monthly', or 'yearly'
    const matchKey = getXAxisKey(); // 'date', 'month', or 'year'
    
    // Get formatted targets for merging later
    const targets = formatSalesTargets();
  
    // For daily data, handle by index rather than matching keys
    if (activeViewType === "daily") {
      return data.map((currentItem, index) => {
        ...
        const matchingTarget = targets.find(target => target[matchKey] === currentItem[matchKey]);
        ...
        return {
          ...
          nobGrowth: prevItem.nob
            ? (((currentItem.nob - prevItem.nob) / prevItem.nob) * 100).toFixed(1)
            : null,
    ...
    // For monthly and yearly data, match by key
      ...
      // Create a map of targets for easy lookup
      const targetMap = {};
      targets.forEach((item) => {
        targetMap[item[matchKey]] = item.target;
      });
      ...
      return data.map((currentItem) => {
        ...
        // Find matching target
        const targetValue = targetMap[currentItem[matchKey]] || null;
  ...
  const formatDateToMatch = (dateString) => {
    ...
    // Adjust this format to match exactly how dates are formatted in your daily data
    return `${date.getDate().toString().padStart(2, '0')}-${(date.getMonth() + 1).toString().padStart(2, '0')}`;
  };
  
  const formatSalesTargets = () => {
    if (!salesTarget || salesTarget.length === 0) return [];
    
    const activeViewType = activeTab; // 'daily', 'monthly', or 'yearly'
    const periodKey = getXAxisKey(); // 'date', 'month', or 'year'
    
    if (activeViewType === 'monthly') {
      // For monthly view, format targets by month
      return salesTarget.map(target => {
        const targetDate = new Date(target.DOT);
        const month = targetDate.toLocaleString('default', { month: 'short' });
        const year = targetDate.getFullYear().toString();
        
        // Only include targets for the selected year
        if (year === selectedYear) {
          return {
            month: month, // Format to match your month format in sales data
            target: parseFloat(target.Amount)
          };
        }
        return null;
      }).filter(item => item !== null);
    } 
    else if (activeViewType === 'daily') {
      // For daily view, we need to map each target to the exact date
      return salesTarget.map(target => {
        // Format date to match your date format in daily data
        const formattedDate = formatDateToMatch(target.DOT);
        return {
          date: formattedDate,
          target: parseFloat(target.Amount)
        };
    ...
    else if (activeViewType === 'yearly') {
      // For yearly view, aggregate targets by year
      const yearlyTargets = {};
      
      salesTarget.forEach(target => {
        const year = new Date(target.DOT).getFullYear().toString();
        if (!yearlyTargets[year]) {
          yearlyTargets[year] = 0;
        }
        yearlyTargets[year] += parseFloat(target.Amount);
      });
      
      return Object.keys(yearlyTargets).map(year => ({
        year: year,
        target: yearlyTargets[year]
      }));
    }
    ...
    <div className="px-6 py-5 sm:px-8 border-b border-gray-100">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-5">
        ...
        <h2 className="text-2xl font-bold text-gray-800 flex items-center group relative">
          <span className="group-hover:text-blue-600 text-blue-600 transition-colors duration-300">Sales Analytics</span>
        ...
        <ResponsiveContainer width="100%" height={400}>
          ...
          >
            ...
            <Tooltip
              ...
              // Update your tooltip formatter to include target
